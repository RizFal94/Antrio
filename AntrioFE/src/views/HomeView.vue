<template>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5 p-8">
    ...
    <div
      class="rounded-lg overflow-hidden h-40 cursor-pointer hover:-translate-y-1 transition-transform duration-300 m-3"
      v-for="service in formData.service"
      :key="service.id"
      @click="klikService(service)"
    >
      <div class="h-full flex flex-col justify-center items-center p-4 bg-green-500">
        <h1 class="font-bold text-white text-center text-lg">{{ service.service }}</h1>
      </div>
    </div>
...
const klikService = (service) => {
  axios.post(`${baseUrl}/ambil-antrian`, {
